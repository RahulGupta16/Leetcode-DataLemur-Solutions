...
cte2 as
(
select *, dense_rank() over(order by challenges_created desc) as chal_rank
from cte),

cte3 as(
select hacker_id, challenges_created
from cte2
where chal_rank in (select chal_rank from cte2 group by chal_rank having count(chal_rank)=1)
union 
select hacker_id, challenges_created
from cte2 where chal_rank = 1)

select cte3.hacker_id, Hackers.name, cte3.challenges_created
from cte3
inner join Hackers 
on cte3.hacker_id = Hackers.hacker_id
